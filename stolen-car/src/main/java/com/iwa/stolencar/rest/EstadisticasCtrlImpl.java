...
public class EstadisticasCtrlImpl implements EstadisticasCtrl {
	...
	@Override
	@GetMapping
	@RequestMapping("/{statisticId}/day")
	public ResponseEntity getDayStatistics(
			@PathVariable(name = "statisticId") Integer stadisticId) {
		logger.info("CTRL: getDayStatistics method");

		List<DiaDTO> diaDTOs = 
				this.estadisticasService.getDayStatistics();
		List<DiaVO> diaVOs = 
				this.estadisticasParser.parseToDiaVOList(diaDTOs);

		if (logger.isTraceEnabled())
			logger.trace("Day statistics: " + diaVOs);

		return new ResponseEntity<>(diaVOs, HttpStatus.OK);
	}
}
