...
public class EstadisticasParserImpl implements EstadisticasParser{
	...
	public EstadisticaVO parseToEstadisticaVO(EstadisticaDTO dto) {
		...
		estadisticaVO.setId(dto.getId());
		estadisticaVO.setName(dto.getName());
		
		return estadisticaVO;
	}

	@Override
	public List<DiaVO> parseToDiaVOList(List<DiaDTO> diaDTOs) {
		List<DiaVO> diaVOs = new ArrayList<>();
		if(diaDTOs == null)
			return diaVOs;
		
		for(DiaDTO diaDTO: diaDTOs){
			diaVOs.add(this.parseToDiaVO(diaDTO));
		}
	...
	public DiaVO parseToDiaVO(DiaDTO diaDTO) {
		...
		DiaVO diaVO = new DiaVO();
		diaVO.setId(diaDTO.getId());
		diaVO.setName(diaDTO.getName());
		diaVO.setCarAmount(diaDTO.getCarAmount());
		diaVO.setMotorcycleAmount(diaDTO.getMotorcycleAmount());
