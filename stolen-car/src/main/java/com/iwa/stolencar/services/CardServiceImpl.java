...
public class CardServiceImpl implements CardService {
	...
	@Override
	public void deleteTransportationCard(Integer id, String cardTitle) throws NotFoundException {
		logger.info("SERVICE: deleteTransportationCard method");
		
		if(logger.isDebugEnabled())
			logger.debug("- Card id : " + id);
		
		if(logger.isDebugEnabled())
			logger.debug("- Card title : " + cardTitle);
		
		if(cardTitle.equals("cars")){
			Auto auto = this.autoDAO.getById(id);
			
			if(auto == null)
				throw new NotFoundException("Auto with id: " + id + " not found");
			
			this.autoDAO.deleteById(id);
		}
		
		if(cardTitle.equals("motorcycles")){
			Moto moto = this.motoDAO.getById(id);
			
			if(moto == null)
				throw new NotFoundException("Moto with id: " + id + " not found");
			
			this.motoDAO.deleteById(id);
		}
